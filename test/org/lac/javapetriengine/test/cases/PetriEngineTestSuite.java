...
import org.lac.javapetriengine.engine.PetriEngine;
import org.lac.javapetriengine.engine.policies.FirstInLinePolicy;
import org.lac.javapetriengine.engine.policies.TransitionsPolicy;
import org.lac.javapetriengine.errors.IllegalTransitionFiringError;
import org.lac.javapetriengine.exceptions.NotInitializedPetriNetException;
...
import org.lac.javapetriengine.petrinets.factory.PetriNetFactory;
import org.lac.javapetriengine.petrinets.factory.PetriNetFactory.petriNetType;
import org.lac.javapetriengine.test.utils.TransitionEventObserver;
...
public class PetriEngineTestSuite {
	 ...
	 */
	private void setUpEngine(String PNML, petriNetType type){
		factory = new PetriNetFactory(PNML);
		petri = factory.makePetriNet(type);
		engine = new PetriEngine(petri, policy);
	 ...
	 * @param PNML path to the PNML file
	 */
	private void setUpEngine(String PNML){
		setUpEngine(PNML, petriNetType.PLACE_TRANSITION);
	}
	...
	public void testFireTransitionWhenNoThreadIsSleeping() {
		...
		setUpEngine(MONITOR_TEST_01_PETRI);
		...
		engine.subscribeToTransition(t1, obs);
		
		try {
			engine.fireTransition(t0);
	...
	public void testFireTransitionWhenAThreadIsSleepingInT2() {
		...
		TransitionEventObserver obs = new TransitionEventObserver();
		engine.subscribeToTransition(t1, obs);
		engine.subscribeToTransition(t2, obs);
	...
	public void testSubscribeToNotInformedTransitionShouldThrowException(){
		try{
			...
			engine.subscribeToTransition(t0, obs);
	 ...
	 */
	@Test
	public void testCreatingEngineWithoutPetriShouldThrowException(){
		try{
			PetriEngine aMonitor = new PetriEngine(null, policy);
			Assert.fail("An exception should've been thrown before this point");
		} catch (Exception e){
			Assert.assertEquals("IllegalArgumentException", e.getClass().getSimpleName());
		}
	 ...
	 */
	@Test
	public void testCreatingEngineWithoutPolicyShouldThrowException(){
		try{
			PetriEngine aMonitor = new PetriEngine(petri, null);
			Assert.fail("An exception should've been thrown before this point");
		} catch (Exception e){
			Assert.assertEquals("IllegalArgumentException", e.getClass().getSimpleName());
		}
	 ...
	 */
	@Test
	public void testEngineShouldSendEventWhenInformedTransitionIsFired(){
		setUpEngine(MONITOR_TEST_02_PETRI);
	 ...
	 */
	@Test
	public void testEngineShouldNoLongerSendEventsAfterUnsubsciption(){
		setUpEngine(MONITOR_TEST_02_PETRI);
		...
		TransitionEventObserver obs = new TransitionEventObserver();
		Subscription sub = engine.subscribeToTransition(t0, obs);
	 ...
	 */
	@Test
	public void testEngineShouldSendEventsOnlyToSubscribedTransition(){
		setUpEngine(MONITOR_TEST_03_PETRI);
	 ...
	 */
	@Test
	public void testEngineShouldFireTransitionOnlyIfGuardAllows(){
		setUpEngine(PETRI_WITH_GUARD_01);
	...
	public void testAutomaticTransitionDisabledDueToGuardShuldBeFiredWhenGuardAllows(){
		...
		setUpEngine(PETRI_WITH_GUARD_02);
	...
	public void testThreadSleepingInTransitionDisabledByInhibitionShouldWakeUpWhenInhibitionDissapears(){
		...
		setUpEngine(PETRI_WITH_INHIBITOR_01);
	...
	public void testPerennialFiringShouldNotSendAThreadToSleepWhenTransitionIsDisabled(){
		...
		Assert.assertFalse(engine.getQueuesState()[1]);
	}
	 ...
	 */
	@Test
	public void testEngineShouldAcceptTransitionNameAsFireArgument(){
		...
		try {
			engine.fireTransition(t0.getName());
